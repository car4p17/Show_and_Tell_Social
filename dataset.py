    ...
    if not os.path.exists(config.temp_annotation_file):
        ...
        for ann_id in coco.anns:
            ...
            if id < 1000:
                ...
                should_add = entry['image_id'] in coco.imgs and os.path.exists(os.path.join(config.train_image_dir, coco.imgs[entry['image_id']]['file_name']))
                if should_add:
...
def prepare_eval_data(config):
    ...
    image_ids = [image_id 
                    for image_id in list(coco.imgs.keys())
                    if os.path.exists(os.path.join(config.eval_image_dir, coco.imgs[image_id]['file_name']))]
